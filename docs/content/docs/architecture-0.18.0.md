---
title: "Architecture 0.18.0"
slug: "architecture-0.18.0"
weight: 99
draft: true
---
...
`Retry-After` gives the client a concrete backoff hint, instead of leaving it to guess (see Configuration for `maxQueuedWriters`, and Concurrency handling in Go for the queue itself). `DELETE /events` in dev mode gets the same `503`/`Retry-After` treatment, since it joins the same queue (see Dev mode).
...
`DELETE /documents`, with no `{type}`, is the same operation widened to every type at once: a shortcut for a total rebuild (every projection, not just one) instead of one `DELETE /documents/{type}` call per type. Same rationale, same lack of a dev-mode gate: the whole documents store is reconstructible from events, so wiping all of it is a normal rebuild step, not a destructive one.
...
|---|---|
| `socketPath` | `TAMARACKDB_SOCKET_PATH` |
...
`devMode` (see Configuration) turns on two things, neither reachable otherwise, both meant only for local development and test environments, never a production instance: `DELETE /events`, and Go's standard profiling endpoints under `/debug/pprof/` (CPU, heap, goroutine, and so on). Neither exists at all unless `devMode` is `true`. Left at its default of `false`, a request to either gets the stdlib's plain `404`, like any other unregistered path. That keeps them out of reach in a normal deployment, instead of reachable-but-guarded, matching the "fail loud, keep it simple" stance used throughout: there's no separate permission or confirmation step once `devMode` is on.

`DELETE /events` wipes every event, identifier, and metadata row; the schema itself stays in place, and documents are untouched (see Documents). It responds `204 No Content` on success. It joins the same FIFO write-admission queue as `POST /write` (see Concurrency handling in Go), so a wipe can no longer land mid-write, or a write mid-wipe, and it counts toward `maxQueuedWriters` the same way. The in-memory Sequence Position counter is deliberately left alone by a wipe: the tables go empty, but the counter keeps climbing from wherever it was.

The profiling endpoints are read-only and outside the write-admission queue: they inspect the running process (CPU samples, memory allocations, goroutine stacks), not the database, so they carry none of `DELETE /events`'s data-loss risk. They're still dev-mode-only because a CPU or heap profile can reveal details about the data flowing through a live request that a production deployment shouldn't expose to whoever can reach the port.

`DELETE /documents/{type}` (see Documents) is not gated by `devMode`, unlike `DELETE /events`: it clears one document type, reconstructible from events, the normal first step of a rebuild, not a whole-database wipe.
...
- `tamarackdb_requests_queued` (gauge): number of write requests (`POST /write`, or, in dev mode, `DELETE /events`) currently waiting in the queue
