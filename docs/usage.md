...
(the queue manager, the SQLite schema, concurrency handling), see
...
above); the append is rejected if any event matching it exists after
...
| 503 | `AppendQueueFull` | `append`/`DELETE /` only: the write queue is already at `maxQueuedWriters`; retry after the `Retry-After` header |
...
- `GET /metrics`: Prometheus exposition format, whether a writer is currently active,
  requests currently queued, longest current queue wait, and write/failure counters.
- `GET /debug`: a JSON snapshot of the current active writer, if any, and every queued
  writer with its wait time.
...
1.2ms`. This lets you watch your `read`/`append` calls go by without printing anything
