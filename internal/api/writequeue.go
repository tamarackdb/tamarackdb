...
// DELETE /events, and DELETE /documents/{type}. On success the caller
// owns the returned *queue.Turn and must call Done exactly once
// (typically via defer). On failure,
...
// immediately.
func (s *Server) joinWriteQueue(w http.ResponseWriter, r *http.Request) (*queue.Turn, bool) {
	turn, err := s.qm.Join(r.Context(), queue.KindTransaction)
	if err != nil {
		s.handleErr(w, r, err)
		return nil, false
	}
